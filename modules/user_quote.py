...
def get_quotes(phenny, input):
    if input.group(2) is None:
        quotes_string = "\n".join([u"<{}> {}".format(nick, quote.encode("utf-8", "replace")) for nick, quotes in phenny.quotes.items() for quote, submitter in quotes])
    else:
        nick = input.group(2).lower()
        quotes_string = "\n".join([u"<{}> {}".format(nick, quote.encode("utf-8", "replace")) for quote, submitter in phenny.quotes.get(nick, [])])
    if quotes_string:
