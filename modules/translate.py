...
def tr(phenny, context): 
   ...
   _, input, output, phrase = context.groups()

   phrase = phrase.encode('utf-8')

   if (len(phrase) > 350) and (not context.admin): 
      return phenny.say('Phrase must be under 350 characters.')

   input = input or detect(phrase)
   if not input: 
      err = 'Unable to guess your language, sorry.'
      return phenny.say(err)
   ...
   if input != output: 
      ...
      phenny.say(msg)
   else: phenny.say('Language guessing failed, try suggesting one.')

tr.rule = (['tr','translate'], ur'(?:([a-z]{2}) +)?(?:([a-z]{2}) +)?["“](.+?)["”] *$')
tr.example = '.tr "parlez-vous français?" or .tr en fr "I don\'t speak french"'
tr.priority = 'low'
